defmodule DistributedOrdersWeb.OrderLiveTest do
  use DistributedOrdersWeb.ConnCase

  import Phoenix.LiveViewTest
  import DistributedOrders.OrdersFixtures

  @create_attrs %{documents_url: "some documents_url", item_name: "some item_name", total_qty: 42}
  ...
  describe "Index" do
    ...
    test "lists all orders", %{conn: conn, order: order} do
      ...
      assert html =~ order.documents_url
    end
    ...
    test "saves new order", %{conn: conn} do
      ...
      assert html =~ "some documents_url"
    end
    ...
    test "updates order in listing", %{conn: conn, order: order} do
      ...
      assert html =~ "some updated documents_url"
    end
